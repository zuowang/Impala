...
//void TestScalarGet(int batch_size, void* d) {
//  TestData* data = reinterpret_cast<TestData*>(d);
//  for (int j = 0; j < batch_size; ++j) {
//    for (int bit_idx = 0; bit_idx < data->bit_index.size(); ++bit_idx) {
//      data->result.push_back(data->bm.Get<false>(data->bit_index[bit_idx]));
//    }
//  }
//}
//
//void TestAVX2Get(int batch_size, void* d) {
//  TestData* data = reinterpret_cast<TestData*>(d);
//  uint32_t result[] = {0, 0, 0, 0, 0, 0, 0, 0};
//  for (int j = 0; j < batch_size; ++j) {
//    for (int64_t bit_idx = 0; bit_idx < data->bit_index.size(); bit_idx += 8) {
//      data->bm.Get<false>(&data->bit_index[bit_idx], result);
...
//      data.bit_index.push_back((1 << (6 + i)) + bit_idx);
//      data.bm.Set<false>((1 << (6 + i)) + bit_idx, (i + bit_idx) % 2 == 0);
//    }
//  }
...
//  Benchmark suite("bitmap get");
//  suite.AddBenchmark("AVX2", TestAVX2Get, &data);
//  suite.AddBenchmark("Scalar", TestScalarGet, &data);
