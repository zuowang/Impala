      ...
      switch (current_page_header_.data_page_header.definition_level_encoding) {
        ...
        case parquet::Encoding::FLE: {
          ...
          fle_def_levels_.reset(new FleDecoder(data_, num_definition_bytes, bit_width,
              current_page_header_.data_page_header.num_values));
          break;
        }
...
inline bool HdfsParquetScanner::BaseColumnReader::ReadValue(MemPool* pool, Tuple* tuple) {
  ...
  return ReadSlot(tuple->GetSlot(slot_desc()->tuple_offset()), pool);
}
...
Status HdfsParquetScanner::AssembleRows(int row_group_idx) {
  ...
  while (!reached_limit && !cancelled && rows_read < expected_rows_in_group) {
    ...
    if (num_column_readers > 0) {
      ...
      if (conjunct_ctxs_.size() == 0 && !conjunct_check_) {
        bool result = true;
        Tuple* firstTuple = tuple;
        int i = num_rows;
        while (i > 10) {
          InitTuple(template_tuple_, tuple);
          result = column_readers_[0]->ReadValue(pool, tuple);
        ...
        for (int c = 1; c < num_column_readers; ++c) {
          ...
          int i = num_rows;
          while (i > 10) {
            result = column_readers_[c]->ReadValue(pool, tuple);
            tuple = next_tuple(tuple);
            result = column_readers_[c]->ReadValue(pool, tuple);
            tuple = next_tuple(tuple);
            result = column_readers_[c]->ReadValue(pool, tuple);
...
//          }
      ...
      } else if (simple_predicates_.size() != 0) {
        num_rows = 0;
        bool result = true;
...
void HdfsParquetScanner::CreateSimplePredicates() {
  vector<void*> column_readers;
  for (int i = 0; i < column_readers_.size(); ++i) {
    column_readers.push_back(reinterpret_cast<void*>(column_readers_[i]));
  }
